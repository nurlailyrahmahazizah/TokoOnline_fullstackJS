...
const namaTable = `pack`

// funtion untuk ambil data customer
exports.ambilDataPack = () => {
...
exports.ambilDataDenganParameter = (parameter) => {
    return new Promise((resolve,reject) => {
        ...
        let query = `select*from pack where ${params}`
        // dijalankan query nya
        connection.query(query, (error,result) =>{
            if(error){
                reject(error)
...
exports.tambahPack = (pack) => {
    return new Promise((resolve, reject) => {
        // ambil key dari object customer
        let key = Object
            .keys(pack) // [key1, key2, dst]
            .join() // "key1, key2, dst"  

        // ambil value dari object customer
        let value = Object
            .keys(pack) // [key1, key2, dst]
            .map(item => `"${pack[item]}"`) // ["value1", "value2", dst]
            .join() // `"value1", "value2", dst`
        
        let query = `insert into pack (${key}) values (${value})`
...
exports.ubahPack = (data, parameter) => {
    return new Promise((resolve,reject) => {
        ...
        // menyusun query
        let query = `update pack set ${perubahanData} where ${params}`
...
exports.delete = (parameter) => {
    return new Promise((resolve, rejected) => {
        ...
        let query = `delete from pack where ${params}`
